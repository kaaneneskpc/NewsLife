      ...
      Text(text = "Top News",fontWeight = FontWeight.SemiBold)
       LazyColumn{
            items(articles.size){index->
                TopNewsItem(articles = articles[index],
...
fun TopNewsItem(articles: Articles,onNewsClick: () -> Unit = {},) {
    Box(modifier = Modifier
        ...
        Column(modifier = Modifier
            ...
            Text(text = MockData.stringToDate(articles.publishedAt!!).getTimeAgo(),color = Color.White,fontWeight = FontWeight.SemiBold)
            Spacer(modifier = Modifier.height(100.dp))
            Text(text = articles.title!!,color = Color.White,fontWeight = FontWeight.SemiBold)
        }
    }
}
